...
# import demjson
import torch
from .general import get_config_root
from .distributed_utils import is_main_process, print_only_main
...
class Configuration:
    ...
    def load_yaml(self, file):
        with open(file, "r") as stream:
            ...
            pythia_root_dir = get_config_root()
...
def load_default_config(extra_args=None):
    default_yml = os.path.join(get_config_root(), 'default.yml')
    config = Configuration(default_yml).freeze().get_config()
    if extra_args is not None:
        config.update_with_args(extra_args)
    return config
