...
from ..optim import loss_func_with_clutter
...
def eval_loss(pred, feature_map, inter_module, clutter_bank, use_z=False, down_sample_rate=8,
    ...
    assert loss_type == 'with_clutter', 'Only loss functions with clutter is supported'
    loss_func = loss_func_with_clutter
...
def solve_pose_with_kp_features(feature_map, inter_module, kp_features, clutter_bank, poses,
                                ...
                                verbose=False, down_sample_rate=8, hierarchical=0):
    ...
    if len(px_samples) == 21:
        l, n = 3, 7
    elif len(px_samples) == 9:
        l, n = 1, 9
    elif len(px_samples) == 12:
        l, n = 4, 3
    elif len(px_samples) == 5:
        l, n = 5, 1
    else:
        raise NotImplementedError
    ...
    corr = torch.cat(all_corr, dim=-2).detach().cpu().numpy().reshape(-1)
    ...
    if not disable_p and hierarchical > 0:
        ...
        h_px_samples = np.linspace(px - dx * hierarchical, px + dx * hierarchical, (2 * hierarchical + 1), endpoint=True)
        h_py_samples = np.linspace(py - dy * hierarchical, py + dy * hierarchical, (2 * hierarchical + 1), endpoint=True)
        ...
        kp_coords = np.repeat(np.expand_dims(input_kp_coords, axis=1), len(principal_samples), axis=1)  # (num_poses, num_px x num_py, 1024, 2)
        ...
        kp_vis = np.repeat(np.expand_dims(input_kp_vis, axis=1), len(principal_samples), axis=1)  # (num_poses, num_px x num_py, 1024)
    ...
    C = camera_position_from_spherical_angles(pred['distance_pre'], pred['elevation_pre'], pred['azimuth_pre'], degrees=False, device=device)
    C = torch.nn.Parameter(C, requires_grad=True)
    ...
    pred['principal'] = [pred['px'] * down_sample_rate + hm_w*down_sample_rate / 2, pred['py'] * down_sample_rate + hm_h*down_sample_rate / 2]
